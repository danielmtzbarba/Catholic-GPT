...
class ChatBotUI:
    ...
    def create_widgets(self):
        ...
        self.chat_canvas = tk.Canvas(
            self.chat_frame, bg="#ffffff", highlightthickness=0
        )
    ...
    def add_message(self, sender, message, user=True):
        ...
        card.pack(
            fill=tk.X, padx=10, pady=5, anchor="e" if user else "w", ipadx=5, ipady=5
        )
        ...
        sender_label = tk.Label(
            card, text=f"{sender}:", font=self.bold_font, bg="#f0f0f0"
        )
        ...
        message_text.pack(anchor="w", padx=10, pady=(0, 5), fill=tk.X)

        # Dynamic lines
        # # Dynamically calculate height based on message length
        avg_chars_per_line = 90
        lines = message.count("\n") + (len(message) // avg_chars_per_line) + 1
        message_text.config(height=min(100, lines))  # Up to 15 lines visible per card
